...
template <typename p_type, typename v_type, typename v_flow>
class Simulation {
  ...
  struct VectorField {
    array<Fixed<P, K>, deltas.size()> v[N][M];

    Fixed<P, K>& add(int x, int y, int dx, int dy, Fixed<P, K> dv) {
      return get(x, y, dx, dy) += dv;
    }

    Fixed<P, K>& get(int x, int y, int dx, int dy) {
  ...
  static VectorField<32, 16> velocity{}, velocity_flow{};
  // TODO: find a way to use sizes directly from compile options or runtime

  tuple<v_type, bool, pair<int, int>> propagate_flow(int x, int y, v_type lim) {
    last_use[x][y] = UT - 1;
    v_type ret = 0;
    for (auto [dx, dy] : deltas) {
      int nx = x + dx, ny = y + dy;
